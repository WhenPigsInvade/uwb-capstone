...
float prevAvgWeight = NAN;
unsigned long prevAverageTime = 0;  
...
// --- Timing ---
const unsigned long MEASURE_INTERVAL = 5000UL;;
const unsigned long AVERAGE_INTERVAL = 20UL * 60UL * 1000UL;
...
struct SensorAccumulator {
  ...
  int countAmbientTemp    = 0;
  int countHumidity       = 0;
  int countAmbientTempOut = 0;
  int countHumidityOut    = 0;
  int countCoilTempTop    = 0;
  int countCoilTempMid    = 0;
  int countCoilTempBot    = 0;
  int countDewPointIn     = 0;
  int countDewPointOut    = 0;
  int count               = 0;
...
void takeMeasurement() {
  ...
  float dewPointIn = calculateDewPoint(ambientTemp, humidity);
  if (!isnan(dewPointIn)) { acc.dewPointIn += dewPointIn; acc.countDewPointIn++; }
  ...
  float dewPointOut = calculateDewPoint(ambientTempOut, humidityOut);
  if (!isnan(dewPointOut)) { acc.dewPointOut += dewPointOut; acc.countDewPointOut++; }

  float rawWeight = newDataReady ? LoadCell.getData() : lastRawWeight;
  newDataReady = false;
  lastRawWeight = rawWeight;

  if (!isnan(rawWeight)) {
    acc.weightSum   += rawWeight;
    acc.countWeight++;
  }   

  acc.count++;

  if (!isnan(ambientTemp))    { acc.ambientTemp    += ambientTemp;    acc.countAmbientTemp++; }
  if (!isnan(humidity))       { acc.humidity       += humidity;       acc.countHumidity++; }
  if (!isnan(ambientTempOut)) { acc.ambientTempOut += ambientTempOut; acc.countAmbientTempOut++; }
  if (!isnan(humidityOut))    { acc.humidityOut    += humidityOut;    acc.countHumidityOut++; }
  if (!isnan(coilTempTop))    { acc.coilTempTop    += coilTempTop;    acc.countCoilTempTop++; }
  if (!isnan(coilTempMid))    { acc.coilTempMid    += coilTempMid;    acc.countCoilTempMid++; }
  if (!isnan(coilTempBot))    { acc.coilTempBot    += coilTempBot;    acc.countCoilTempBot++; }
  ...
  Serial.print("Dew point inside   : "); Serial.print(dewPointIn); Serial.println(" C");
  Serial.print("Dew point outside   : "); Serial.print(dewPointOut); Serial.println(" C");
  Serial.print("Raw weight          : "); Serial.print(rawWeight); Serial.println(" g");
...
void printAndSendAverages() {
  ...
  float avgAmbientTemp    = acc.countAmbientTemp    > 0 ? acc.ambientTemp    / acc.countAmbientTemp    : nan("");
  float avgHumidity       = acc.countHumidity       > 0 ? acc.humidity       / acc.countHumidity       : nan("");
  float avgAmbientTempOut = acc.countAmbientTempOut > 0 ? acc.ambientTempOut / acc.countAmbientTempOut : nan("");
  float avgHumidityOut    = acc.countHumidityOut    > 0 ? acc.humidityOut    / acc.countHumidityOut    : nan("");
  float avgCoilTempTop    = acc.countCoilTempTop    > 0 ? acc.coilTempTop    / acc.countCoilTempTop    : nan("");
  float avgCoilTempMid    = acc.countCoilTempMid    > 0 ? acc.coilTempMid    / acc.countCoilTempMid    : nan("");
  float avgCoilTempBot    = acc.countCoilTempBot    > 0 ? acc.coilTempBot    / acc.countCoilTempBot    : nan("");
  float avgDewPointIn = acc.countDewPointIn > 0 ? acc.dewPointIn / acc.countDewPointIn : nan("");
  float avgDewPointOut = acc.countDewPointOut > 0 ? acc.dewPointOut / acc.countDewPointOut : nan("");

  float avgWeight = acc.countWeight > 0
                  ? acc.weightSum / acc.countWeight
                  : 0.0;
                
  // --- Calculate ml/hour ---
  float mlPerHour = NAN;
  unsigned long now = millis();
  if (!isnan(prevAvgWeight) && prevAverageTime > 0) {
    float weightDeltaG   = avgWeight - prevAvgWeight;  // grams change
    float weightDeltaMl  = weightDeltaG;               // 1g = 1ml for water
    float timeDeltaHours = (float)(now - prevAverageTime) / 3600000.0;
    if (timeDeltaHours > 0) {
      mlPerHour = weightDeltaMl / timeDeltaHours;
    }
  }
  // store this window for next calculation
  prevAvgWeight    = avgWeight;
  prevAverageTime  = now;

  // --- Print averages to serial ---
  Serial.println("=== 20-Min Averages ===");
  Serial.print("Samples attempted   : "); Serial.println(acc.count);
  Serial.print("Ambient temp inside : "); Serial.print(avgAmbientTemp);    Serial.print(" C ("); Serial.print(acc.countAmbientTemp);    Serial.println(" samples)");
  Serial.print("Humidity inside     : "); Serial.print(avgHumidity);       Serial.print(" % ("); Serial.print(acc.countHumidity);       Serial.println(" samples)");
  Serial.print("Ambient temp outside: "); Serial.print(avgAmbientTempOut); Serial.print(" C ("); Serial.print(acc.countAmbientTempOut); Serial.println(" samples)");
  Serial.print("Humidity outside    : "); Serial.print(avgHumidityOut);    Serial.print(" % ("); Serial.print(acc.countHumidityOut);    Serial.println(" samples)");
  Serial.print("Coil temp top       : "); Serial.print(avgCoilTempTop);    Serial.print(" C ("); Serial.print(acc.countCoilTempTop);    Serial.println(" samples)");
  Serial.print("Coil temp mid       : "); Serial.print(avgCoilTempMid);    Serial.print(" C ("); Serial.print(acc.countCoilTempMid);    Serial.println(" samples)");
  Serial.print("Coil temp bot       : "); Serial.print(avgCoilTempBot);    Serial.print(" C ("); Serial.print(acc.countCoilTempBot);    Serial.println(" samples)");
  Serial.print("Dew point inside   : "); Serial.print(avgDewPointIn); Serial.print(" C ("); Serial.print(acc.countDewPointIn); Serial.println(" samples)");
  Serial.print("Dew point outside   : "); Serial.print(avgDewPointOut); Serial.print(" C ("); Serial.print(acc.countDewPointOut); Serial.println(" samples)");
  Serial.print("Avg weight          : "); Serial.print(avgWeight);
  
  Serial.print("Avg weight          : "); Serial.print(avgWeight);   Serial.print(" g ("); Serial.print(acc.countWeight); Serial.println(" samples)");
  if (!isnan(mlPerHour)) {
    Serial.print("Water rate          : "); Serial.print(mlPerHour); Serial.println(" ml/hr");
  } else {
    Serial.println("Water rate          : calculating... (need 2 windows)");
  }

  Serial.print(" g ("); Serial.print(acc.countWeight); Serial.println(" samples)");
  ...
  json += "{\"sensor_type\":\"dew_point_inside\",\"value\":"     + String(isnan(avgDewPointIn)  ? 0.0 : avgDewPointIn)  + "},";
  json += "{\"sensor_type\":\"dew_point_outside\",\"value\":"    + String(avgDewPointOut)   + "},";
  json += "{\"sensor_type\":\"current_weight\",\"value\":"        + String(avgWeight)        + "},";
  json += "{\"sensor_type\":\"water_rate_ml_hr\",\"value\":"      + String(isnan(mlPerHour) ? 0.0 : mlPerHour) + "},";
...
void setup() {
  ...
  // --- SHT3x inside (CH3) --- temporarily removed
  tcaSelect(3);
  ...
  // --- HX711 init ---
  LoadCell.setSamplesInUse(32);  // average 32 samples — reduces noise
  LoadCell.begin();
  unsigned long stabilizingtime = 5000;  // increased from 2000 to 5000ms
  boolean _tare = true;
